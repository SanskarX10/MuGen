...
import os
import json
import music21 as m21 # convert midi , kern -> m21 -> kern , midi
...
def load_songs_in_kern(dataset_path):
    ...
    songs = []
    # go through all the files in dataset and load with music 21
...
def transpose(song):
    ...
    #get interval for transposition eg B maj -> C maj
    if key.mode == 'major':
        interval = m21.interval.Interval(key.tonic, m21.pitch.Pitch("C"))
    elif key.mode == 'minor':
        interval = m21.interval.Interval(key.tonic, m21.pitch.Pitch("A"))

    # transpose song by calculated inter
...
def encode_song(song, time_step = 0.25):
    # example > p = 60 , d = 1.0 -> [60,"_","_","_].sum(1/4 th node) = 1
...
def preprocess(dataset_path):
    ...
    for i,song in enumerate(songs):
        ...
        song = transpose(song) # transpose songs to C major /A  minor
...
def main():
    ...
    songs = create_single_file_dataset(SAVE_DIRECTORY, SINGLE_FILE_DATASET, SEQUENCE_LENGTH)
